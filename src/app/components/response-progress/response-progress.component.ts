...
export class ResponseProgressComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.isAdmin) {
      ...
      this.selectedDelegateUserInfo &&
      (this.selectedDelegateUserInfo.userJobTitle == 'MGR' ||
        this.selectedDelegateUserInfo.userJobTitle == 'TL')
    ) {
      this.isSendReminderEnabled = true;
    } else if (
      ...
      this.isAdmin ||
      this.userJobTitle == 'MGR' ||
      this.userJobTitle == 'TL'
    ) {
      this.isSendReminderEnabled = true;
    } else {
    ...
    if (
      this.userInformation.canReviewObservations &&
      this.reportCycle != 'SAB Commentary Report'
    ) {
    ...
    //  this.getUserInfo();
    this.getResponseProgressData();
    this.responseProgressInterval = setInterval(
      () => this.checkResponseProgressData(),
      60000
    );
  ...
  getResponseProgressData() {
    ...
    let url =
      'InProgController/InProgWorkItems?isAdmin=' +
      this.isAdmin +
      '&userLogin=';
    if (this.selectedDelegateUserInfo) {
      url =
        ...
        '&departmentCode=' +
        this.departmentCode +
        '&isDelegatedUser=true&onBehalfOf=' +
        this.selectedDelegateUserInfo.loginId +
        '&behalfUserTitle=' +
        this.selectedDelegateUserInfo.userJobTitle +
        '&behalfUserDeptCode=' +
        this.selectedDelegateUserInfo.departmentCode +
        '&r=' +
      ...
      if (this.userJobTitle == 'SEC') {
        ...
        url =
          ...
          this.departmentCode +
          '&isDelegatedUser=false&onBehalfOf=' +
          this.userInformation.sabMember.loginId +
          '&r=' +
          (Math.floor(Math.random() * 100) + 100);
      }
    }
    ...
    this.coreService.get(url).subscribe(
      async (response: any) => {
        ...
        let filterSequence = localStorage.getItem(
          'sabResponseProgressFilterSequence'
        );
        let filterDepartment = localStorage.getItem(
          'sabResponseProgressFilterDepartment'
        );
        let filterStatus = localStorage.getItem(
          'sabResponseProgressFilterStatus'
        );
        let filterDirectorate = localStorage.getItem(
          'sabResponseProgressFilterDirectorate'
        );
        let filterBehalf = localStorage.getItem(
          'sabResponseProgressFilterBehalf'
        );
        let filterMultipleDept = localStorage.getItem(
          'sabResponseProgressFilterMultipleDept'
        );
        let filterReminderCount = localStorage.getItem(
          'sabResponseProgressFilterReminderCount'
        );
        let filterReviewed = localStorage.getItem(
          'sabResponseProgressFilterReviewed'
        );
        ...
        response.map((data: any, i: any) => {
          ...
          data.stepCustomId =
            data.stepUnqName != 'GPA-Pending-Extraction'
              ? data.stepCustomId
              : data.obsId;
        ...
        if (filterType != null) {
          ...
          let filterValue = await this.applyOldFilter(
            filterType,
            this.typeList,
            []
          );
  ...
  masterToggle() {
    ...
    if (this.isAllSelected()) {
      ...
      this.dataSource.data.forEach((row: any, i) => {
        ...
        this.dataSource.data[i].isSelected = this.dataSource.data[
          i
        ].hasOwnProperty('isSelected')
          ? !this.dataSource.data[i].isSelected
          : true;

        // this.dataSource.data[i].isSelected = true;
        let stepCustomId =
          row.stepUnqName != 'GPA-Pending-Extraction'
            ? row.stepCustomId
            : row.obsId;
        ...
        this.selectedRow.map((data) => {
          if (
            data.stepUnqName == 'G&PAApprove' ||
            data.stepUnqName == 'G&PAReview' ||
            data.stepUnqName == 'MGR-PENDING-COMBINING' ||
            data.stepUnqName == 'G&PACOombineResponse' ||
            data.stepUnqName == 'SN-CEO-APPROVAL-COMBINE' ||
            data.stepUnqName == 'GPA-ASSIGN' ||
            data.stepUnqName == 'G&PA Park' ||
            data.stepUnqName == 'G&PAApprove-EditResponse' ||
            data.stepUnqName == 'GPA-Pending-Extraction'
          ) {
            find = true;
            this.isSendReminder = false;
  ...
  navigateTo(row: any, event: MouseEvent) {
    ...
    if (this.isAdmin && event?.ctrlKey) {
      ...
      this.router.navigate(
        ['response-progress/work-item-details', row.stepCustomId],
        {
          queryParams: {
            obsId: row.obsId,
            stepUnqName: row.stepUnqName,
  ...
  sequenceChange(event: any) {
    if (!this.isSequenceFirst) {
      if (event.source.value == 'All') {
        ...
        this.isSequenceFirst = true;
        let filter: any = localStorage.getItem(
          'sabResponseProgressFilterSequence'
        );
        ...
        if (!event.source._selected) {
          ...
          if (filter) {
            setTimeout(() => {
              ...
              if (
                this.sequenceData.value.length + 1 ==
        ...
        if (!event.source._selected) {
          ...
          if (filter) {
            setTimeout(() => {
              ...
              if (
                this.departmentData.value.length + 1 ==
    ...
    if (!this.isStatusFirst) {
      if (event.source.value == 'All') {
        ...
        if (event.source._selected) {
          ...
          localStorage.setItem(
            'sabResponseProgressFilterStatus',
            JSON.stringify(this.statusList)
          );
        }
        this.isStatusFirst = false;
      } else {
    ...
    if (!this.isOnBehalfFirst) {
      if (event.source.value == 'All') {
        ...
        if (event.source._selected) {
          ...
          localStorage.setItem(
            'sabResponseProgressFilterBehalf',
            JSON.stringify(this.onBehalfList)
          );
        }
        this.isOnBehalfFirst = false;
      } else {
        ...
        if (!event.source._selected) {
          ...
          if (filter) {
            setTimeout(() => {
              ...
              if (
                this.onBehalfData.value.length + 1 ==
                this.onBehalfList.length
              ) {
                this.isOnBehalfFirst = true;
                this.onBehalfData.setValue(this.onBehalfList);
                localStorage.setItem(
    ...
    if (!this.isMultipleFirst) {
      if (event.source.value == 'All') {
        ...
        if (event.source._selected) {
          ...
          localStorage.setItem(
            'sabResponseProgressFilterMultipleDept',
            JSON.stringify(this.multipleList)
          );
        }
        this.isMultipleFirst = false;
      } else {
        ...
        if (!event.source._selected) {
          ...
          if (filter) {
            setTimeout(() => {
              ...
              if (
                this.multipleData.value.length + 1 ==
                this.multipleList.length
              ) {
                this.isMultipleFirst = true;
                this.multipleData.setValue(this.multipleList);
                localStorage.setItem(
    ...
    if (!this.isReviewedFirst) {
      if (event.source.value == 'All') {
        ...
        if (event.source._selected) {
          ...
          localStorage.setItem(
            'sabResponseProgressFilterReviewed',
            JSON.stringify(this.reviewedList)
          );
        }
        this.isReviewedFirst = false;
      } else {
        ...
        if (!event.source._selected) {
          ...
          if (filter) {
            setTimeout(() => {
              ...
              if (
                this.reviewedData.value.length + 1 ==
                this.reviewedList.length
              ) {
                this.isReviewedFirst = true;
                this.reviewedData.setValue(this.reviewedList);
                localStorage.setItem(
    ...
    if (!this.isReminderCountFirst) {
      if (event.source.value == 'All') {
        ...
        if (event.source._selected) {
          ...
          localStorage.setItem(
            'sabResponseProgressFilterReminderCount',
            JSON.stringify(this.reminderCountList)
          );
        }
        this.isReminderCountFirst = false;
      } else {
        ...
        if (!event.source._selected) {
          ...
          if (filter) {
            setTimeout(() => {
              ...
              if (
                this.reminderCountData.value.length + 1 ==
                this.reminderCountList.length
              ) {
                this.isReminderCountFirst = true;
                this.reminderCountData.setValue(this.reminderCountList);
                localStorage.setItem(
  ...
  onPaginateChange(event: any) {
    ...
    localStorage.setItem(
      'sabPaginatorResponseProgressIndex',
      JSON.stringify(this.pageIndex)
    );
  }
  ...
  checkResponseProgressData() {
    ...
    this.coreService.get(url).subscribe(
      (response: any) => {
        ...
        if (JSON.stringify(response: any) === JSON.stringify(this.inboxItems)) {
          this.typeData = new FormControl();
          ...
          let isNADepartmentAvailable = false;
          let isNADirectorateAvailable = false;
          let filterType = localStorage.getItem(
            'sabResponseProgressFilterType'
          );
          let filterSequence = localStorage.getItem(
            'sabResponseProgressFilterSequence'
          ...
          response.map((data: any, i: any) => {
            ...
            this.reviewedList.push(data.reviewed);
            reminderCountList.push(data.reminderCount);
            let changedSelected = this.dataSource.data.filter((item) => {
              return (
                (data.stepCustomId == item.stepCustomId &&
                  data.stepUnqName != 'GPA-Pending-Extraction') ||
                (data.obsId == item.obsId &&
                  data.stepUnqName == 'GPA-Pending-Extraction')
              );
            });
            if (changedSelected.length > 0) {
              data['isSelected'] = changedSelected[0].hasOwnProperty(
                'isSelected'
              )
                ? changedSelected[0].isSelected
                : false;
            }
            // data.isExpanded = true;
          });
          ...
          if (filterType != null) {
            isFilterSelected = true;
            let filterValue = this.applyOldFilter(
              filterType,
              this.typeList,
              oldTypeList
            );
  ...
  generateDraft() {
    ...
    let url =
      'ReportController/generateDraft?reportYear=' +
      this.reportYear +
      '&reportCycle=' +
      this.reportCycle +
      '&obsIds=' +
  ...
  selectionChange(data: any, row: any) {
    ...
    for (let i = 0; i < this.dataSource.data.length; i++) {
      if (
        (this.dataSource.data[i].stepCustomId == row.stepCustomId &&
          data.stepUnqName != 'GPA-Pending-Extraction') ||
        (this.dataSource.data[i].obsId == row.obsId &&
          data.stepUnqName == 'GPA-Pending-Extraction')
      ) {
        this.dataSource.data[i].isSelected = this.dataSource.data[
          i
        ...
        if (this.dataSource.data[i].isSelected) {
          ...
          for (let j = 0; j < this.selectedRow.length; j++) {
            if (
              (this.selectedRow[j].stepCustomId ==
                this.dataSource.data[i].stepCustomId &&
                data.stepUnqName != 'GPA-Pending-Extraction') ||
              (this.selectedRow[j].obsId == this.dataSource.data[i].obsId &&
                data.stepUnqName == 'GPA-Pending-Extraction')
            ) {
    ...
    this.selectedRow.map((data: any) => {
      ...
      if (
        data.stepUnqName == 'NORMAL-DCEO-OR-CEO-APPROVAL' ||
        data.stepUnqName == 'G&PAApprove' ||
        data.stepUnqName == 'G&PAReview' ||
        data.stepUnqName == 'MGR-PENDING-COMBINING' ||
        data.stepUnqName == 'G&PACOombineResponse' ||
  ...
  _onEditResponse(result: any): void {
    ...
    let url =
      'InProgController/editResponse?stepCustomId=' +
      stepIds +
      '&userName=' +
      this.userName +
      '&comment=' +
  ...
  onContextMenuAction1(row: any) {
    let serverHost = location.href.split('/#/');
    if (
      row.responseStatus == 'Pending Combine & Complete Response' ||
      ...
      let url =
        'workItemController/getResponseCount?stepCustomId=' + row.stepCustomId;
      this._loading.setLoading(true, url);
      this.coreService.get(url).subscribe(
        (response: any) => {
          this._loading.setLoading(false, url);
