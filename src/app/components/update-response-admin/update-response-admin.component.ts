...
export class UpdateResponseAdminComponent implements OnInit {
  ...
  checkSelected(value :string){
    ...
    // } else {
    if (this.responseData.obsType == 'NEW') {
      if (this.selectedDateString == 'متباين الرأى بشأنها') {
        this.classification = 'C';
      } else if (this.selectedDateString == 'تتعلق بجهات حكومية/اخرى') {
        this.classification = 'E';
      } else if (this.selectedDateString == 'تم الانتهاء من إجراءاتها')
  ...
  onUpdateResponse(): void {
    ...
    if (this.isFileError) {
      ...
      this.dialogRef.close({ event: 'updateResponseByAdmin', data: sendResponseData });
    }
  }
